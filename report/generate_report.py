"""Builds report/REPORT.md from the actual metrics.json produced by train.py,
so the numbers in the report always match the last training run."""
import json
...
report = f"""# BehaviorGuard - AI-Powered Behavioral Anomaly Detection
...
- **False positive rate @ top 1% analyst alert budget:** {test['fp_rate_at_budget']:.3f}
- **Anomaly recall captured @ top 1% alert budget:** {test['recall_at_budget']:.3f}
- **Binary anomaly-vs-normal Average Precision (PR-AUC):** {test['pr_auc']:.3f}
- **Validation-split PR-AUC (sanity check, no leakage):** {val['pr_auc']:.3f}
